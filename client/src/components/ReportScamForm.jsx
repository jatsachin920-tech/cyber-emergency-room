...
const ReportScamForm = ({ onScamReported }) => {
  ...
  return (
    <div className="bg-[#0d1527]/80 border border-slate-800/60 backdrop-blur-md p-4 sm:p-6 rounded-xl shadow-2xl relative font-sans mt-6 sm:mt-11 mx-1 sm:mx-0 box-border">
      ...
      <h3 className="text-sm sm:text-base font-bold text-slate-200 mb-4 flex items-center gap-2 uppercase tracking-wide">
        <span className="text-red-700">🚨</span> Broadcast Threat Report
      </h3>

      <form onSubmit={handleSubmit} className="space-y-4 text-xs sm:text-sm">
        <div>
          <label className="block text-slate-400 uppercase tracking-wider text-[10px] sm:text-xs font-medium mb-1.5">
            Scam Classification
          </label>
          <select 
            value={formData.threatType}
            onChange={(e) => setFormData({...formData, threatType: e.target.value})}
            className="w-full bg-slate-950 border border-slate-800/80 rounded-xl p-3 text-slate-300 focus:outline-none focus:ring-1 focus:ring-red-500 transition-all cursor-pointer font-medium text-xs sm:text-sm"
          >
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-slate-950 border border-slate-800/80 rounded-xl p-3 text-slate-300 placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-red-500 transition-all resize-none leading-relaxed text-xs sm:text-sm"
          ></textarea>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-red-600 cursor-pointer hover:bg-red-500 text-white font-bold uppercase tracking-widest p-3 rounded-xl transition-all shadow-[0_0_20px_rgba(220,38,38,0.15)] disabled:opacity-40 active:scale-[0.99] text-xs sm:text-sm"
        >
          {loading ? '📡 BROADCASTING INJECTOR...' : '⚡ BROADCAST ALERT'}
        </button>

        {status.text && (
          <div className={`p-2.5 rounded-lg border text-center transition-all animate-pulse text-[11px] sm:text-xs font-medium box-border ${
            status.type === 'success' 
